 ...
 * 获取城市建议（用于自动完成）
 */
export function getCitySuggestions(input: string, limit: number = 10): string[] {
  if (!input) return [];
  ...
  return CHINA_CITIES
    .filter(city => 
      ...
      Object.entries(CITY_ALIASES).some(([alias, normalizedCity]) => 
        alias.toLowerCase().includes(lowerInput) && normalizedCity === city
      )
    )
    .slice(0, limit);
}
